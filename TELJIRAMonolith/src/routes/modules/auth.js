...
router.post(
  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty())
      return res
        .status(400)
        .json({ error: "ValidationError", details: errors.array() });
    ...
    try {
      ...
      await auditLog(req, "user.register", "user", result.rows[0].id, {
        email,
      });
      ...
      return res
        .status(400)
        .json({ error: "BadRequest", message: "User exists or invalid data" });
    }
  },
);
